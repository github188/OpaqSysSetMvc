...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    mobile: {
        validator: function (value, param) {
            return /^((\(\d{2,3}\))|(\d{3}\-))?(13|15)\d{9}$/.test(value);
        },
        message: '手机号码不正确'
    },
    loginName: {
...
$.fn.datagrid.defaults.loadFilter = function (data) {
    ...
    if (opt.useBM) {
        totalBM = opt.totalBM;
        rowsBM = opt.rowsBM;
        return { total: data[totalBM], rows: data[rowsBM] };
    }
    else if (opt.pagination == false)
...
$.modalDialog = function (options) {
    if ($.modalDialog.handler == undefined || ($.modalDialog.href != undefined && options.href != undefined && $.modalDialog.href != options.href)) {// 避免重复弹出        
        var opts = $.extend({
            title: '',
            width: 840,
            height: 680,
            ...
            onClose: function () {
                if (options.handler != undefined)
                    $.modalDialog.handler = options.handler;
                else
                    $.modalDialog.handler = undefined;
                if ($.modalDialog.parenthref != undefined) {
                    var temp = $.modalDialog.parenthref;
                    $.modalDialog.href = temp;
                    $.modalDialog.parenthref = undefined;
                }
                else {
                    $.modalDialog.href = undefined;
                }
                $(this).dialog('destroy');
            },
            onOpen: function () {
        ...
        if ($.modalDialog.href != undefined) {
            var temp = $.modalDialog.href;
            $.modalDialog.parenthref = temp;
        }
        //加载完成，关闭进度窗口
